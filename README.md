...
=======
Di seguito trovi **la stessa documentazione**, **senza cambiare alcun contenuto**, ma **riscritta come README chiaro, leggibile e strutturato**, adatto a chi entra nel progetto e deve capire rapidamente *come funziona* e *come usarlo*.

---

# Market Monitor – Architettura e Uso

Questo progetto permette di **comporre dinamicamente una strategia di monitoraggio e trading** partendo da una configurazione YAML.
L’architettura è modulare, orientata a thread asincroni e facilmente estendibile con nuove strategie, sorgenti dati e output.
...
* istanzia il `Builder`;
* avvia la strategia;
* avvia tutti i thread di I/O asincroni configurati.

---

### Builder

**`src/market_monitor/builder.py`**

Responsabile della costruzione dell’intero runtime.

Funzioni principali:

* inizializza il logging;
* inizializza (opzionalmente) le connessioni Oracle e Timescale;
* carica dinamicamente la strategia utente indicata in `load_strategy_info`;
* costruisce il contenitore di dati real-time (`RTData`);
* inietta `RTData` nella strategia tramite `set_market_data`;
* configura:

  * code trade (sincrone o asincrone);
  * thread di distribuzione dati (Bloomberg, Excel, Redis, file trade);
* crea le GUI configurate (Tkinter, Excel, Dummy) e le associa tramite `set_gui`;
* restituisce:

  * l’istanza della strategia;
  * la lista dei thread da avviare.

---

### Live Data Hub

**`src/market_monitor/live_data_hub/real_time_data_hub.py`**

Fornisce `RTData`, un contenitore **thread-safe** che gestisce:

* book di mercato;
* stato interno ed eventi;
* blob dati condivisi;
* utility per:

  * calcolo mid price;
  * conversione FX in EUR;
  * gestione delle sottoscrizioni (Bloomberg / Redis);
  * routing dei dati verso le strategie.

---

### Input Threads

**`src/market_monitor/input_threads/`**

Thread dedicati all’ingestione dei dati:
...
Questi thread alimentano `RTData` o la coda trade.

Gestione specifica Bloomberg:

* `event_handler/BBGEventHandler.py`
  adatta i messaggi Bloomberg al formato interno.

---

### Strategy Framework

**`src/market_monitor/strategy/StrategyUI`**

* `StrategyUIAsync`

  * orchestra i task asincroni:

    * update HF / LF;
    * gestione trade;
    * osservazione parametri dinamici;
  * invoca le callback definite dalla strategia.
* `StrategyUI`

  * scheletro concreto da estendere per implementare strategie utente.

---

### Publishers

**`src/market_monitor/publishers/`**

Gestiscono l’output dei dati prodotti dalla strategia verso:

* Redis;
* altri canali configurabili.
...
* esempi di strategie Fixed Income / ETF;
...
Le classi strategiche sono caricate dinamicamente tramite il blocco `load_strategy_info` del file YAML.
...
* strategie dimostrative (es. `TestTradeManagerStrategy`);
* riferimento pratico per l’integrazione end-to-end.
...
* creare una classe in un path raggiungibile dal `package_path` indicato nel YAML;
* estendere `StrategyUI`;
* implementare le callback necessarie.

---

### 2. Configurare il file YAML

Esempio: `testing/real_strategy_with_mock.yaml`
...
* **`market_monitor.tasks`**
  ...
  * task attivi:
    ...
    * `trade`
    * `dynamic_params`
  * frequenze e modalità sincrona/asincrona della coda trade
* **`*_data_distributor` / `trade_distributor`**

  * sorgenti dati attive (Bloomberg, Excel, Redis, file);
  * parametri specifici (es. DB file trade, connessioni);
* **`logging`**

  * livelli console e file;
...
### 3. Avviare il monitor

```bash
python -m market_monitor.run_monitor <config.yaml>
```

Oppure importando `run_monitor` da uno script Python.

---

### 4. Gestione della chiusura

La strategia dovrebbe implementare `on_stop` per:

* chiudere il `TradeManager`;
* fermare publisher;
* chiudere GUI;
* rilasciare risorse.
...
Le callback più comuni da implementare in una sottoclasse di `StrategyUI`:

* **`on_market_data_setting()`**
  chiamata quando `RTData` viene assegnato; utile per impostare le sottoscrizioni (es. Bloomberg).

* **`wait_for_book_initialization() -> bool`**
  ritorna `True` quando il book è pronto (es. mid non NaN).
  Viene controllata in loop prima di avviare i task asincroni.

* **`on_book_initialized()`**
  hook di notifica quando il book è pronto.

* **`update_HF()`**
  logica ad alta frequenza (NAV, indicatori, ecc.).
  Può restituire DataFrame + metadata per l’esportazione GUI.

* **`update_LF()`**
  logica a bassa frequenza (controlli periodici, salvataggi).

* **`on_trade(trades: pd.DataFrame)`**
  gestione dei trade di mercato provenienti dalla coda `q_trade`.

* **`on_my_trade(trades: pd.DataFrame)`**
  gestione dei trade proprietari (`OWN`).

* **`on_config_change(key, old_value, new_value)`**
  chiamata dal watcher dei parametri dinamici.

* **`on_other_thread_start()`**
  invocata all’avvio dei thread ausiliari.

* **`on_start_strategy()` / `on_stop()`**
  lifecycle della strategia.

Tutte le callback sono orchestrate da `StrategyUIAsync` in base al blocco `tasks` del YAML.

---

## Uso del Trade Manager

**`user_strategy/utils/TradeManager/trade_manager.py`**

Gestore thread-safe dei trade con persistenza append-only.
...
TradeManager(
    ...
    model_prices=None,
    time_zero_lag=10.0,
    trade_folder=Path("data/trades"),
    max_time_to_match_side=10.0,
    auto_save_interval=500,
    engine="pyarrow",
    compression="snappy",
    enable_persistence=True,
    use_timezone_aware=True
)
```

* richiede un `book_storage` con metodo `get_last_before`;
* può avviare un thread `TimeZeroPLManager` per il PL istantaneo.
...
* assegna il side usando il mid del book;
* calcola spread PL e PL di modello;
* separa i trade proprietari.

---

### Query disponibili

* `get_trades(n_of_trades=None)`
* `get_my_trades`
* `get_trades_from_isin`
* `get_trades_from_ticker`
* `get_filtered_trades`

I DataFrame sono ordinati per timestamp decrescente.
La cache interna è invalidata automaticamente all’arrivo di nuovi trade.

---

### Persistenza

* salvataggio giornaliero su Parquet:

  * `trades_YYYYMMDD.parquet`
* ricostruzione automatica a startup se `enable_persistence=True`.
...
Va chiamato dentro `on_stop` della strategia.

---

## Esempio minimo di strategia
>>>>>>> c56ea32 (Update project structure and configs)
...
=======
        # esporta latest verso GUI / publisher
>>>>>>> c56ea32 (Update project structure and configs)
...
=======
Con una configurazione YAML che punta a `MyStrategy`, `run_monitor`:

* inizializza il book;
* avvia i thread di input;
* gestisce i trade;
* garantisce la persistenza tramite `TradeManager`.

---

Se vuoi, nel prossimo passo posso:

* ridurre ulteriormente il README in versione **“quick start”**;
* separare **architettura** e **uso** in due file;
* aggiungere diagrammi concettuali (thread, flusso dati, lifecycle).
>>>>>>> c56ea32 (Update project structure and configs)
