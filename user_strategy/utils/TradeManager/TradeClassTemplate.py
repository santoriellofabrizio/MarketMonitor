    ...
    def build_trades_obj(trades: pd.DataFrame):
        for row in trades.itertuples():
            ...
            if row.own_trade != 0:
                ...
                yield MyTrade(ticker, row.isin, row.last_update, row.quantity, row.price, row.market, row.currency, side, getattr(row,"price_multiplier", 1))
            else:
                yield Trade(ticker, row.isin, row.last_update, row.quantity,  row.price, row.market, row.currency, getattr(row, "price_multiplier", 1))
