...
class CreditTradeAnalysis(StrategyUI):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        # Risoluzione Conflitto Init: Sorgente prezzi e Messaging
        ...
        if excel_path and excel_path.exists():
            ...
            # Uso dei valori dell'index direttamente per gli ISIN
            self.all_isin = [str(i).upper().strip() for i in self.instruments_df.index]
        else:
        ...
        if rabbit_cfg.get('activate', False):
            ...
            print(f"==== Rabbit export -> channel: {self.rabbit_exporting_channel} ====")
        ...
        if redis_export_cfg.get('activate', False):
            ...
            print(f"==== Redis export -> channel: {self.redis_exporting_channel} ====")
    ...
    def subscribe_bloomberg(self):
        markets_mapping = {
            "ETFP": "IM", "XAMS": "NA", "XPAR": "FP",
            "MOTX": "MILA", "ETLX": "ETLX"
        }
        for markets, symbols in ((self.etf_markets, self.etf_isins), (self.bond_markets, self.bond_isins)):
            for m in markets:
                for s in symbols:
                    ...
                    self.global_subscription_service.subscribe_bloomberg(id=f"{m}_{s}", subscription_string=sub_string)

    def subscribe_kafka(self):
        fields = {"BID": "bidBestLevel.price", "ASK": "askBestLevel.price",
                  "BID_SIZE": "bidBestLevel.quantity", "ASK_SIZE": "askBestLevel.quantity"}
        for markets, symbols in ((self.etf_markets, self.etf_isins), (self.bond_markets, self.bond_isins)):
            for m in markets:
                base = f"COALESCENT_DUMA.{m}"
                for s in symbols:
                    self.global_subscription_service.subscribe_kafka(id=f"{m}_{s}", symbol_filter=s,
                                                                     topic=f"{base}.BookBest", fields_mapping=fields)
                    for ev in ("PublicDeal", "Trade"):
                        self.global_subscription_service.subscribe_kafka(id=f"{m}_{s}:{ev}", symbol_filter=s,
                                                                         topic=f"{base}.{ev}")
    ...
    def publish_trades_on_dashboard(self, new_trades):
        # Risoluzione Conflitto publish: mappatura descrizione e filtro colonne
        if new_trades.empty:
            return
        ...
        if self.redis_trade_dashboard_messaging:
            self.redis_trade_dashboard_messaging.export_message(channel=self.redis_exporting_channel, value=new_trades,
        ...
        if self.rabbit_trade_dashboard_messaging:
            self.rabbit_trade_dashboard_messaging.export_message(channel=self.rabbit_exporting_channel,
                                                                 value=new_trades, date_format='iso', orient="records")
    ...
    def get_live_data(self):
        ...
        # Risoluzione Conflitto get_live_data: logica raggruppamento prezzi per ISIN/Valuta
        grouped: dict[str, dict[str, list]] = {}
        for instrument_id, price in mid.items():
            parts = instrument_id.split("_", 1)
            isin = parts[1] if len(parts) > 1 else parts[0]
