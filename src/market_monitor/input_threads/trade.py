...
class TradeThread(threading.Thread):
    ...
    Thread that react when a new trade is received. On event read from a DB generated by MarketTrades, and puts new
     ...
     """
    def __init__(self, queue: Queue | asyncio.Queue, path: str, locker: threading.Lock = None):
        super().__init__()
    ...
    def put_in_queue(self, trade_type: TradeType, item: pd.DataFrame):
        """
        Aggiunge l'elemento alla coda in modo sicuro.
        Se la coda è piena applica drop-oldest: scarta il trade più vecchio
        per fare spazio al nuovo, loggando un warning di backpressure.
        Un trade vecchio processato con ritardo produce P&L errato.
        """
        if self.asynchronous:
            asyncio.run_coroutine_threadsafe(self.queue_trade.put((trade_type, item)), self.loop)
        else:
            if self.queue_trade.full():
                try:
                    dropped = self.queue_trade.get_nowait()
                    logger.warning(
                        f"[BACKPRESSURE] Trade queue piena (maxsize={self.queue_trade.maxsize}), "
                        f"dropped trade type={dropped[0]}"
                    )
                except Exception:
                    pass
            try:
                self.queue_trade.put_nowait((trade_type, item))
            except Exception:
